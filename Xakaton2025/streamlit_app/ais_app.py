...
def render_dashboard():
    ...
    with col1:
        ...
        # Нормализуем категории (убираем пробелы) и считаем только положительные значения
        raw_counts = {}
        for t in st.session_state.tasks:
            c = str(t.get('Категория', 'Не указана')).strip() or 'Не указана'
            raw_counts[c] = raw_counts.get(c, 0) + 1
        # Отфильтруем нули (на всякий случай) и отсортируем
        items = [(k, v) for k, v in raw_counts.items() if v and v > 0]
        items.sort(key=lambda x: x[1], reverse=True)
        # Если категорий слишком много, сгруппируем хвост в «Другое»
        MAX_CATS = 10
        if len(items) > MAX_CATS:
            head = items[:MAX_CATS-1]
            tail_sum = sum(v for _, v in items[MAX_CATS-1:])
            items = head + [('Другое', tail_sum)]
        if items:
            df_cat_all = pd.DataFrame({'Категория': [k for k, _ in items], 'Количество': [v for _, v in items]})
            ...
            fig1.update_layout(legend_title_text='Категория')
...
def render_settings():
    ...
    with col1:
        if st.button("🗑️ Очистить все заявки", type="secondary"):
            # Очистка в БД (tasks + связанные назначения)
            clear_tasks_in_db()
            st.session_state.tasks = load_tasks_from_db()
            st.session_state.assignments = load_assignments_from_db()
            st.success("✅ Все заявки удалены из БД!")
            st.rerun()
    
    with col2:
        if st.button("🗑️ Очистить всех исполнителей", type="secondary"):
            # Очистка в БД (executors + связанные назначения)
            clear_executors_in_db()
            st.session_state.executors = load_executors_from_db()
            st.session_state.assignments = load_assignments_from_db()
            st.success("✅ Все исполнители удалены из БД!")
            st.rerun()
