...
def render_dashboard():
    ...
    if st.session_state.tasks:
        ...
        with col2:
            st.markdown("### 📊 Загрузка исполнителей")
            executor_data = []
            for executor in st.session_state.executors:
                if executor.get('active', True):
            ...
            if executor_data:
                ...
                fig = px.bar(
                    ...
                    x='name',
                    y='assigned',
                    title="Количество назначенных заявок",
                    color='assigned',
        ...
        st.info("📝 Пока нет заявок. Создайте первую заявку в разделе 'Заявки'.")
    ...
    if editing_task_id:
        ...
        with col3:
            if st.button("🗑️ Удалить заявку", type="secondary"):
                st.session_state.tasks = [t for t in st.session_state.tasks if t['id'] != editing_task_id]
                st.session_state.assignments = [a for a in st.session_state.assignments if a.get('task_id') != editing_task_id]
                st.session_state[f"editing_task_{editing_task_id}"] = False
            ...
            with st.container():
                ...
                with col3:
                    if st.button(f"🗑️ Удалить", key=f"delete_{task['id']}"):
                        ...
                        st.session_state.assignments = [a for a in st.session_state.assignments if a.get('task_id') != task['id']]
                        st.success("✅ Заявка удалена!")
                        st.rerun()
...
def render_executors_management():
    ...
    if editing_executor_id:
        ...
        with col3:
            if st.button("🗑️ Удалить исполнителя", type="secondary"):
                st.session_state.executors = [e for e in st.session_state.executors if e['id'] != editing_executor_id]
                st.session_state.assignments = [a for a in st.session_state.assignments if a.get('executor_id') != editing_executor_id]
                st.session_state[f"editing_executor_{editing_executor_id}"] = False
    ...
    if st.button("👥 Добавить исполнителя", type="primary"):
        if executor_name and executor_email:
            ...
            st.session_state.executors.append(new_executor)
            ...
            with st.container():
                ...
                with col3:
                    if st.button(f"🗑️ Удалить", key=f"delete_exec_{executor['id']}"):
                        st.session_state.executors = [e for e in st.session_state.executors if e['id'] != executor['id']]
                        st.session_state.assignments = [a for a in st.session_state.assignments if a.get('executor_id') != executor['id']]
                        st.success("✅ Исполнитель удален!")
                        st.rerun()
...
def render_settings():
    ...
    with col2:
        ...
        auto_refresh = st.checkbox("Автообновление дашборда", value=True)
